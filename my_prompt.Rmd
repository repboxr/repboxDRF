...
Ok, I now want to implement file caches to the direct replication format (DRF).

I want you to propose the concrete code modifications:

1. In repboxStata add:

- A function repbox_always_cache_cmd. That returns a vector of Stata commands after which we always want to generate a cache file of the current data set. Currently it shall just return c("merge"), i.e. we want to auto cache after merge commands.

- In inject.R add an injection that writes cache files. This will be the data set after the command was written as dta (using save). The folder shall be project_dir/repbox/stata/cached_dta. Filename simply: cache_{donum}_{line}_{counter}.dta

- After replication was run, generate a simple cache_files.Rds in project_dir/repbox/stata/cached_dta with columns runid, donum, line, counter, cache_file name.

2. In repboxDRF add:

a) Move cached_dta from the repbox folder to project_dir/drf/cached_dta but rename the file as simply {runid}_cache.dta. Don't copy, just move cache file.

b) Have a new R script that can incorporate caches by adapting path_df (possibly also run_df) in an drf object given the cache files in the project_dir/drf/cached_dta directory. There can be differen ways to adapt:

1. If we don't need any scalars, r(...), results from before the cache, we can simply start the path with the runid for which we have the cache with the first command loading the cache.


2. The procedure 1 might be not feasible, if we need scalars or r() results computed before the cached command after the cached command. If that is the case do not shorten the path and ignore the cache.

3. Path shortening short work for both on whether we want later perform a Stata or R analysis in direct replication format.

4. Create in a separate R script functionality that given all paths in a DRF object due to caches, determines whether some files in drf are not needed anymore and can be removed. This will be relevant if we have shortened path in an DRF object using caches and may thus not need some original files anymore.

c) The code for b) should not be tied to the way how cache files are generated in repboxStata but just look at which cache files exist in project_dir/drf/cached_dta. That is because later we may add different routes that also add cache files. 

Discuss and explain your proposed changes first before given the code file modifications. 
