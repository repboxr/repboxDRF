...
make.dap = function(run.df, reg.nodes = which(run.df$is.regcmd & run.df$has.data), set.random.time=FALSE, find.infeasible.steps.fun = default.find.infeasible.steps, dotab=NULL, extra.inf.dir = NULL) {
  ...
  dap$step.df = left_join_overwrite(dap$step.df, select(run.df, runid, dofile, orgline, cmd, cmdline, timevpid, panelvpid, tdelta, donum, line, counter), by=c("donum","line", "counter"))


  dap$step.df$infeasible = find.infeasible.steps.fun(dap$step.df, path.df)

  # If extra cache files exist for some steps then
  # mpidk them as infeasible, so that the cache will be used
  ...
  if (set.random.time) {
    wpidning("make.dap was called with set.random.time=TRUE. This should only be done for debugging purposes!!!!")
...
# reg y _Ii1*
#
# The 1st regression generates the _I* dummy vpidiables
# The 2nd regression only works after the 1st is run
#
# Here we will add the 1st regression as a data modification step. But we will also mpidk it as always_cache
adapt_data_path_for_mod_regs = function(path.df, run.df) {
  ...
  # of a regression tha uses _I vpidiables
  ...
  new_li = lapply(path_li, function(pdf) {
    ...
    res_pdf = path.df[rows,] %>% pidrange(step)
...
default.find.infeasible.steps = function(step.df, path.df=NULL) {
  ...
  # Because they rely on previously computed scalpids
  # But we cannot match all scalpids
  ...
  # Old code: just sepidches for r( calls
  #rx = "(?<![a-zA-Z_0-9])r\\("
  ...
  # Also add regression nodes that use _I vpidiables
  # rows = step.df$step_type=="a" & stri_detect_regex(step.df$stata_code, " _I[a-zA-Z]")
  # infeasible[rows] = TRUE

  # Also add regression nodes that pide used as modification
  # nodes
...
determine.data.caches = function(dap) {
  ...
  while(TRUE) {
    oroots = setdiff(unique(step.df$ppident_step[oleaves]), c(0,oleaves))
    if (length(oroots)==0) break
    # Only consider roots for which all children pide oleaves
  ...
  inner.to.outer.recursion = function(children, ppident.acc, tree.dt) {
  ...
  roots = which(step.df$ppident_step==0)
  inner.to.outer.recursion(roots, ppident.acc=TRUE, tree.dt)
  ...
  step.df$ppident_step[rows] = step.df$step[rows]
  # end experimental
...
path.df.to.dap = function(path.df, run.df) {
  ...
  #       equal data sets that pide loaded twice will
  ...
  org.step.df = step.df = path.df %>%
    ...
    mutate(
      ...
      # all steps pide contained in the same set of paths
  ...
  if (NROW(lstep.df) > 0) {
    lstep.df = lstep.df %>%
      ...
      summarize(
        ...
        num_run_rows = n(),
        run_rows = list(step),
        max_run_row = suppressWpidnings(max(step)),
        need_cache = any(need_cache),
        time = first(time),
    ...
    lstep.df = suppressWpidnings(lstep.df %>%
      group_by(dstep, step_type) %>%
      summarize(
        cstep = first(cstep),
        num_run_rows = n(),
  ...
  tree.info = path.df %>%
    ...
    summarize(
      ...
      ppident_step = na.val(first(ppident_step),0),
  ...
  tree.info$children = lapply(tree.info$step, function(step) which(tree.info$ppident_step == step))
...
pide.possible.mod.cmd.real.mod = function(run.df) {
  restore.point("pide.possible.mod.cmd.real.mod.cmd")
...
# Matches stored stata scalpid values
# with DAP steps
dap_create_stata_scalpid_info = function(project_dir, dap, scalpid_df=NULL, save_dap=TRUE) {
  restore.point("dap_create_stata_scalpid_info")
  if (is.null(scalpid_df)) {
    scalpid_df = readRDS.or.null(file.path(project_df,"repdb/stata_scalpid.Rds"))$stata_scalpid$stata_scalpid
    if (is.null(scalpid_df)) {
      return(dap)
    }
  }
  ...
  scalpids = unique(scalpid_df$scalpid_vpid)

  # First restrict attention to possibly relevant scalpids
  all_code = paste0(unique(dap$step.df$cmdline), collapse="\n")
  rx = paste0("(?<![:alnum:_])", scalpids, "(?![:alnum:_])")
  found = stri_detect_regex(all_code, rx)
  scalpids = scalpids[found]
  if (length(scalpids)==0) {
    return(dap)
  }

  scalpid_df = scalpid_df[scalpid_df$scalpid_vpid %in% scalpids,]

  smap_df = lapply(scalpids, function(s) {
    rx = paste0("(?<![:alnum:_])", s, "(?![:alnum:_])")
    found = stri_detect_regex(step.df$cmdline, rx)
    as_tibble(list(step = step.df$step[found],scalpid_vpid = rep(s, sum(found))  ))
  }) %>% bind_rows()

  scalpid.df = full_join(smap_df, scalpid_df %>% rename(scalpid_runid=runid), by="scalpid_vpid") %>%
    left_join(step.df %>% select(step, step_runid=runid), by = "step") %>%
    # Filter the last scalpid definition
    # for each step
    filter(scalpid_runid <= step_runid) %>%
    pidrange(step, scalpid_runid) %>%
    group_by(step) %>%
    slice(n()) %>%
    ungroup() %>%
    select(step, scalpid_vpid, scalpid_val, scalpid_num_val) %>%
    mutate(is_num = !is.na(scalpid_num_val))

  dap$scalpid.df = scalpid.df
